...
class SlotAttention(nn.Module):
    ...
    def forward(self, features, slots_init, num_iter=3, labels=None):
        ...
        if labels is not None:
            # labels shape: [B, K]
            # attn_mask = (labels == labels.max()).float().unsqueeze(-1)
            # attn_mask = attn_mask * (-1e14)
            attn_mask = 0
        else:
            attn_mask = 0.0
        ...
        for i in range(num_iter):
            ...
            attn = F.softmax(attn_logits + attn_mask, dim=1)
...
class SlotAttentionEncoder(nn.Module):
    def __init__(
        ...
        self.mlp = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(feature_size * 4, slot_size)
        )

        self.slot_attention = SlotAttention(slot_size, slot_size * 4, truncate, drop_path=drop_path)
        ...
        elif init_method == 'text':
            pass
        else:
            raise NotImplementedError
    
    def forward(self, f, sigma, slots_init=None, labels=None):
        B = f.shape[0]
        f = self.mlp(f)
        if slots_init == None:
            # initialize slots.
        ...
        slots, attn  = self.slot_attention(f, slots_init, self.num_iter, labels=labels)
